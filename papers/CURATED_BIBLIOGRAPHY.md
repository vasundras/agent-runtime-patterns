...
Each entry below has been verified by cross-checking title, authors, and identifier (arXiv ID / DOI / proceedings URL) against the canonical source. arXiv IDs resolve at `https://arxiv.org/abs/<id>`.
...
2. **Life Beyond Distributed Transactions: An Apostate's Opinion**
   - Authors: Pat Helland
...
3. **In Search of an Understandable Consensus Algorithm (Raft)**
...
2. **NeMo Guardrails: A Toolkit for Controllable and Safe LLM Applications with Programmable Rails**
...
3. **GuardAgent: Safeguard LLM Agents by a Guard Agent via Knowledge-Enabled Reasoning**
...
5. **GuardAgent: Safeguard LLM Agents by a Guard Agent via Knowledge-Enabled Reasoning**
   - Authors: Zhen Xiang, Linzhi Zheng, Yanjie Li, et al.
   - Year: 2024
   - Venue: arXiv preprint
   - arXiv ID: `2406.09187`
...
All arXiv IDs above have been verified by direct lookup at `https://arxiv.org/abs/<id>`. DOI / conference proceedings entries have been cross-checked against the publisher (ACM DL, USENIX, journal homepage). When a paper had multiple versions (v1, v2, …), the citation refers to the canonical record identified by the bare arXiv ID.
